...
void main() {
  group('DonationModal', () {
    ...
    testWidgets('renders correctly', (tester) async {
      ...
      expect(find.text('Support this argument with a donation'), findsOneWidget);
    ...
    testWidgets('calls processDonation when form is valid', (tester) async {
      ...
      when(() => mockDonationCubit.processDonation(
      ...
      verify(() => mockDonationCubit.processDonation(
            nodeId: 'test_node_id',
            amount: 10.0,
          )).called(1);
    });

    testWidgets('closes modal and calls onDonationComplete when donation succeeds',
        (tester) async {
      // Set up a mock cubit that will emit success
      final successState = DonationState(
      ...
      final cubitState = mockDonationCubit.state as DonationState;
      when(() => mockDonationCubit.state).thenReturn(cubitState.copyWith(
        status: DonationStatus.success,
        amount: 10.0,
      ));
